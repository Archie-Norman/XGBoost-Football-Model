...
# Load the CSV file
results = ("folder with all the historical data")


# Select only the required columns, including the additional ones
results = results[['Div', 'Date', 'Time', 'HomeTeam', 'AwayTeam', 'FTHG', 'FTAG', 'B365CH', 'B365CD', 'B365CA']]

# Rename the columns
results.columns = ['competition_id', 'date', 'time', 'home_team', 'away_team', 'home_score', 'away_score', 'B365CH', 'B365CD', 'B365CA']

# Add a new column 'venue' with all values set to 'unknown'
results['venue'] = 'unknown'

# Change the date format to YYYY-MM-DD
results['date'] = pd.to_datetime(results['date'], format='%d/%m/%Y').dt.strftime('%Y-%m-%d')

# Rearrange the columns to match the desired order
results = results[['date', 'time', 'venue', 'home_team', 'home_score', 'away_team', 'away_score', 'competition_id', 'B365CH', 'B365CD', 'B365CA']]
...
transformed_df = transformed_df[['timestamp', 'venue', 'team', 'against', 'away_or_home', 'goals', 'conceded','competition_id','B365H', 'B365D', 'B365A']]
...
transformed_df = transformed_df[['timestamp', 'venue', 'team', 'against', 'away_or_home', 'goals', 'conceded', 'win_or_loss','competition_id','B365H', 'B365D', 'B365A']]
...
transformed_df = transformed_df[['timestamp', 'venue', 'team', 'against', 'away_or_home', 'goals', 'conceded', 'win_or_loss', 'points','conceded_before_game','goals_before_game','competition_id','B365H', 'B365D', 'B365A']]
...
transformed_df = transformed_df[['timestamp', 'venue', 'team', 'against', 'away_or_home', 'goals', 'conceded', 'win_or_loss', 'points', 'elo_rating', 'points_before_game' ,'conceded_before_game','goals_before_game','competition_id','B365H', 'B365D', 'B365A']]
...
print(data[['B365H_home', 'B365D_home', 'B365A_home']])

data.to_csv("historical output", index=False)
