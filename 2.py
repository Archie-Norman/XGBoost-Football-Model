...
results = pd.read_csv("C:/Users/archi/Desktop/sports betting/Saves data/api-data.csv")
...
results = results[results['timestamp'] <= pd.Timestamp.now() + pd.Timedelta(days=4)]
...
# Sort by timestamp
transformed_df = transformed_df.sort_values(by=['timestamp'])
# Function to calculate the slope (trend) for each team
def calculate_trend(group):
    # Use the index as x and Elo as y for linear regression
    x = np.arange(len(group))  # Sequential indices
    y = group['last_games_elo'].values  # Elo values
    
    # Apply linear regression
    slope, intercept, r_value, p_value, std_err = linregress(x, y)
    return slope

# Compute max Elo up to each date for each team
transformed_df['max_elo_so_far'] = transformed_df.groupby('team')['last_games_elo'].expanding().max().reset_index(level=0, drop=True)
# Compute max Elo up to each date for each team
transformed_df['min_elo_so_far'] = transformed_df.groupby('team')['last_games_elo'].expanding().min().reset_index(level=0, drop=True)
transformed_df['median_elo_so_far'] = transformed_df.groupby('team')['last_games_elo'].expanding().median().reset_index(level=0, drop=True)
transformed_df['mean_elo_so_far'] = transformed_df.groupby('team')['last_games_elo'].expanding().mean().reset_index(level=0, drop=True)
transformed_df['std_elo_so_far'] = transformed_df.groupby('team')['last_games_elo'].expanding().std().reset_index(level=0, drop=True)
#transformed_df['trend_elo_so_far'] = transformed_df.groupby('team').apply(lambda group: group.expanding().apply(calculate_trend, raw=False)).reset_index(level=0, drop=True)



print(f"trans{transformed_df}")
print(f"trans")
...
# Create the 'season' column by determining the season range
min_year, max_year = transformed_df['year'].min(), transformed_df['year'].max()
season = f"{min_year}-{max_year}"

# Assign the same season to all rows (if applicable)
transformed_df['season'] = season

transformed_df.to_csv("cleaned-data.csv", index=False)
